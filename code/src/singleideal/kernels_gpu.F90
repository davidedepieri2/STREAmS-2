...
contains
  ...
  subroutine count_weno_cuf(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin, eul_jmax,&
  ...
  & count_weno_y, count_weno_z)
    ...
    integer, dimension(0:nx,0:ny,0:nz,1:3), device :: ep_ord_change_gpu
    integer :: i,j,k,ishk,ii,jj,kk,iercuda

    count_weno_x = 0._rkind
    !$cuf kernel do(3) <<<*,*>>> reduce(+:count_weno_x)
    do k=1,nz
      do j=1,ny
    ...
    !$cuf kernel do(3) <<<*,*>>> reduce(+:count_weno_y)
    do k=1,nz
    ...
    !$cuf kernel do(3) <<<*,*>>> reduce(+:count_weno_z)
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine count_weno_cuf

  subroutine count_weno_c2_cuf(nv, nv_aux, nx, ny, nz, ng, eul_imin, eul_imax, eul_jmin, eul_jmax,&
  & eul_kmin, eul_kmax, l_base, weno_scheme, sensor_threshold, w_aux_gpu, ep_ord_change_gpu ,&
  & lmax_tag_gpu, wall_tag_gpu, count_weno_x, count_weno_y, count_weno_z)
    ...
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng,1-ng:nz+ng,1:nv_aux), device :: w_aux_gpu
    integer, dimension(0:nx,0:ny,0:nz,1:3), device :: ep_ord_change_gpu
    integer, dimension(1-ng:nx+ng), device :: lmax_tag_gpu
    integer, dimension(1-ng:nx+ng), intent(in), device :: wall_tag_gpu
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine count_weno_c2_cuf

  attributes(global) launch_bounds(256) subroutine euler_x_fluxes_hybrid_c2_kernel(nv, nv_aux, nx,&
  & ny, nz, ng, eul_imin, eul_imax, lmax_base, nkeep, rgas0, w_aux_gpu, coeff_deriv1_gpu, fhat_gpu,&
  & dcsidxc2_gpu, dcsidxnc2_gpu, dcsidyc2_gpu, dcsidync2_gpu, jac_gpu, mcsijac1_gpu, lmax_tag_gpu,&
  & force_zero_flux_min, force_zero_flux_max, weno_scheme, weno_version, sensor_threshold, weno_size,&
  & cp_coeff_gpu, indx_cp_l, indx_cp_r, ep_ord_change_gpu, calorically_perfect, tol_iter_nr,rho0,u0,&
  &t0)
  ...
  subroutine euler_x_update_c2_cuf(nx, ny, nz, ng, nv, eul_imin, eul_imax, fhat_gpu,fl_gpu,jac_gpu,stream_id)
    integer :: nx,ny,nz,ng,nv,eul_imin,eul_imax
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng,1-ng:nz+ng,1:nv), intent(in), device :: fhat_gpu
    real(rkind), dimension(1:nx,1:ny,1:nz,1:nv), intent(inout), device :: fl_gpu
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng), intent(in), device :: jac_gpu
    integer(kind=cuda_stream_kind) :: stream_id
    integer :: i,j,k,m,iv,iercuda
    ...
    do k=1,nz
      ...
      enddo
    enddo
  endsubroutine euler_x_update_c2_cuf
  ...
  subroutine euler_y_update_c2_cuf(nx, ny, nz, ng, nv, eul_jmin, eul_jmax, fhat_gpu,fl_gpu,jac_gpu,wall_tag_gpu,stream_id)
    integer :: nx,ny,nz,ng,nv,eul_jmin,eul_jmax
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng,1-ng:nz+ng,1:nv), intent(in), device :: fhat_gpu
    real(rkind), dimension(1:nx,1:ny,1:nz,1:nv), intent(inout), device :: fl_gpu
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng), intent(in), device :: jac_gpu
    ...
    do k=1,nz
      ...
      enddo
    enddo
  endsubroutine euler_y_update_c2_cuf
  ...
  attributes(global) launch_bounds(256) subroutine euler_y_hybrid_c2_kernel(nv, nv_aux, nx, ny, nz,&
  & ng, eul_jmin, eul_jmax, lmax_base, nkeep, rgas0, w_aux_gpu, fl_gpu, coeff_deriv1_gpu, fhat_gpu,&
  ...
  attributes(device) subroutine eigenvectors_x_c2(b1, b2, b3, uu, vv, ww, c, ci, h, el, er, ut, dcsidxav, dcsidyav)
  ...
  attributes(device) subroutine eigenvectors_y_c2(b1, b2, b3, uu, vv, ww, c, ci, h, el, er, ut, detadxav, detadyav)
  ...
  subroutine force_rhs_2_c2_cuf(nx, ny, nz, ng, fln_gpu, w_aux_gpu, bulk5g, fluid_mask_gpu, r_curv,&
  & yn_gpu, dxdcsinc2_gpu, dydcsinc2_gpu)
    ...
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dxdcsinc2_gpu, dydcsinc2_gpu
    real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_aux_gpu
    integer, dimension(1-ng:,1-ng:,1-ng:), intent(in), device :: fluid_mask_gpu
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine force_rhs_2_c2_cuf
  ...
  subroutine force_rhs_1_c2_cuf(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulk5,&
  & fluid_mask_gpu, dcsidxnc2_gpu, dcsidync2_gpu, jac_gpu)
    integer, intent(in) :: nx, ny, nz, ng
    real(rkind), dimension(1:), intent(in), device :: yn_gpu
    real(rkind), dimension(1:,1:,1:,1:), intent(in), device :: fln_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dcsidxnc2_gpu, dcsidync2_gpu, jac_gpu
    real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_gpu
    real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_aux_gpu
    integer, dimension(1-ng:,1-ng:,1-ng:), intent(in), device :: fluid_mask_gpu
    real(rkind), dimension(5), intent(out) :: bulk5
    ...
    bulk5(4) = bulk_4
    bulk5(5) = bulk_5
  endsubroutine force_rhs_1_c2_cuf
  ...
  subroutine force_var_2_cuf(nx, ny, nz, ng, w_gpu, w_aux_gpu, tbdiff, fluid_mask_gpu, cv_coeff_gpu,&
  ...
  subroutine force_var_1_c2_cuf(nx, ny, nz, ng, yn_gpu, fln_gpu, w_gpu, w_aux_gpu, bulkt,&
  ...
  & dcsidxnc2_gpu, dcsidync2_gpu)
    ...
    integer, dimension(1-ng:,1-ng:,1-ng:), intent(in), device :: fluid_mask_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: jac_gpu, dcsidxnc2_gpu, dcsidync2_gpu
    real(rkind), intent(out) :: bulkt
    real(rkind) :: bulk_5,rho,rhou,rhov,rhow,rhoe,ri,uu,vv,ww,qq,ee,tt
    ...
    !@cuf iercuda=cudadevicesynchronize()
    bulkt = bulk_5
  endsubroutine force_var_1_c2_cuf
  subroutine force_var_2_c2_cuf(nx, ny, nz, ng, w_gpu, w_aux_gpu, tbdiff, fluid_mask_gpu,&
  & cv_coeff_gpu, indx_cp_l, indx_cp_r, t0, calorically_perfect, jac_gpu)
    ...
    integer, dimension(1-ng:,1-ng:,1-ng:), intent(in), device :: fluid_mask_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: jac_gpu
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine force_var_2_c2_cuf
  ...
  subroutine visflx_div_c2_cuf(nx, ny, nz, ng, visc_order, w_aux_gpu, fl_gpu, coeff_deriv1_gpu,&
  & dcsidxc2_gpu, dcsidyc2_gpu, detadxc2_gpu, detadyc2_gpu, dzitdz_gpu, vis_tag_gpu, wall_tag_gpu,&
  & stream_id)
    ...
    real(rkind), dimension(1:,1:,1:, 1:), intent(inout), device :: fl_gpu
    real(rkind), dimension(1:,1:), intent(in), device :: coeff_deriv1_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dcsidxc2_gpu, detadyc2_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dcsidyc2_gpu, detadxc2_gpu
    real(rkind), dimension(1:), intent(in), device :: dzitdz_gpu
    integer, dimension(1-ng:), intent(in), device :: vis_tag_gpu, wall_tag_gpu
    integer :: i,j,k,l,iercuda
  ...
  endsubroutine visflx_div_c2_cuf
  ...
  subroutine visflx_c2_cuf(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
  & cp_coeff_gpu, calorically_perfect, u0, l0, w_gpu, w_aux_gpu, fl_gpu, coeff_deriv1_gpu,&
  & coeff_deriv2_gpu, dcsidxc2_gpu, detadyc2_gpu, detadxc2_gpu, dcsidyc2_gpu, dzitdz_gpu, dzitdzs_gpu,&
  & dzitdz2_gpu, g1_gpu, g2_gpu, g12_gpu, jac_gpu, wallprop_gpu, vis_tag_gpu, wall_tag_gpu, iblock,&
  & ite_rank_x, itu_rank_x, ite_l, itu_l, teshk )
    ...
    real(rkind), dimension(1:,1:), intent(in), device :: coeff_deriv1_gpu
    real(rkind), dimension(0:,1:), intent(in), device :: coeff_deriv2_gpu
    real(rkind), dimension(1:), intent(in), device :: dzitdz_gpu, dzitdzs_gpu, dzitdz2_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dcsidxc2_gpu, detadyc2_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: detadxc2_gpu, dcsidyc2_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: g1_gpu, g2_gpu, g12_gpu, jac_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cp_coeff_gpu
    integer, dimension(1-ng:), intent(in), device :: vis_tag_gpu, wall_tag_gpu
  ...
  subroutine visflx_nosensor_c2_cuf(nx, ny, nz, ng, visc_order, prandtl, t0, indx_cp_l, indx_cp_r,&
  & cp_coeff_gpu, calorically_perfect, iblock, ite_rank_x, itu_rank_x, ite_l, itu_l, u0, l0, w_gpu,&
  & w_aux_gpu, fl_gpu, coeff_deriv1_gpu, coeff_deriv2_gpu, dcsidxc2_gpu, detadyc2_gpu, detadxc2_gpu,&
  & dcsidyc2_gpu, dzitdz_gpu, dzitdzs_gpu, dzitdz2_gpu, g1_gpu, g2_gpu, g12_gpu, jac_gpu, wallprop_gpu,&
  & vis_tag_gpu, wall_tag_gpu, ortho)
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine visflx_nosensor_c2_cuf

  subroutine sponge_cuf(nx,ny,nz,ng,nv,w_gpu,wfar_gpu,fln_gpu,f_sponge_gpu,j_sponge)
    integer, intent(in) :: nx,ny,nz,ng,nv,j_sponge
    real(rkind), dimension(1-ng:, 1-ng:, 1-ng:, 1:), intent(in), device :: w_gpu
    real(rkind), dimension(1:,1:,1:,1:), intent(inout), device :: fln_gpu
    real(rkind), dimension(nx,nv), intent(in), device :: wfar_gpu
    real(rkind), dimension(ny), intent(in), device :: f_sponge_gpu
    integer :: i,j,k,m,iercuda
  ...
  endsubroutine sponge_cuf

  subroutine limiter_cuf(nx,ny,nz,ng,w_gpu,w_aux_gpu,iblock,kblock,indx_cp_l,indx_cp_r,cv_coeff_gpu,&
  &calorically_perfect,t0, tol_iter_nr,rho_lim, tem_lim, rho_lim_rescale, tem_lim_rescale)
    ...
    real(rkind), intent(in) :: t0, tol_iter_nr
    real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_gpu, w_aux_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
    ...
    !$cuf kernel do(3) <<<*,*>>> reduce(+: n_limited_rho, n_limited_tem)
    do k=1,nz
      do j=1,ny
        do i=1,nx
          rho = w_gpu(i,j,k,1)
  ...
  subroutine filter_cuf(nx,ny,nz,ng,w_gpu,w_aux_gpu,indx_cp_l,indx_cp_r,cv_coeff_gpu,&
  &calorically_perfect,t0,tol_iter_nr,coeff_filter_gpu,jfilter,wall_tag_gpu)
    ...
    real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_gpu, w_aux_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
    real(rkind), dimension(0:4), intent(in), device :: coeff_filter_gpu
  ...
  subroutine sensor_c2_cuf(nx, ny, nz, ng, u0, l0_ducros, w_aux_gpu, iblock, ite_rank_x, itu_rank_x,&
  & ite_l, itu_l, teshk, theta_ij_gpu, theta_threshold, jweno)
    ...
    real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_aux_gpu
    real(rkind), dimension(1:,1:), intent(inout), device :: theta_ij_gpu
    integer :: i,j,k,iercuda
    real(rkind) :: div, omod2
  ...
  endsubroutine sensor_c2_cuf

  subroutine visflx_x_cuf(nx, ny, nz, nv, ng, prandtl, t0, indx_cp_l, indx_cp_r, cp_coeff_gpu,&
  & calorically_perfect, enable_les, les_pr, x_gpu, w_aux_gpu, fl_gpu, fhat_gpu)
      ...
      do j=1,ny
        do i=0,nx
          ...
          if (enable_les>0) then
            mu_sgsf = 0.5_rkind*(w_aux_gpu(i,j,k,11)+w_aux_gpu(i+1,j,k,11))/(x_gpu(i+1)-x_gpu(i))
    ...
    do k=1,nz
      do i=1,nx
        do j=0,ny
          ...
          sigz = sigz*muf
          if (enable_les>0) then
            mu_sgsf = 0.5_rkind*(w_aux_gpu(i,j,k,11)+w_aux_gpu(i,j+1,k,11))/(y_gpu(j+1)-y_gpu(j))
            sigq = ((muf-mu_sgsf)*cploc/prandtl+mu_sgsf*cploc/les_pr)*sigq_tt + sigq_qq*muf
          else
            sigq = (sigq_tt*cploc/prandtl+sigq_qq)*muf
          endif
  ...
  & calorically_perfect, enable_les, les_pr, z_gpu, w_aux_gpu, fl_gpu)
    ...
    do j=1,ny
      do i=1,nx
        do k=0,nz
          ...
          sigz = sigz*muf
          if (enable_les>0) then
            mu_sgsf = 0.5_rkind*(w_aux_gpu(i,j,k,11)+w_aux_gpu(i,j,k+1,11))/(z_gpu(k+1)-z_gpu(k))
            sigq = ((muf-mu_sgsf)*cploc/prandtl+mu_sgsf*cploc/les_pr)*sigq_tt + sigq_qq*muf
          else
            sigq = (sigq_tt*cploc/prandtl+sigq_qq)*muf
          endif
  ...
  attributes(global) subroutine bc_nr_lat_x_c2_kernel(start_or_end, nr_type, nx, ny, nz, ng, nv,&
  & w_aux_gpu, w_gpu, fl_gpu, dcsidx_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, winf_gpu, jac_gpu,&
  & mcsi_gpu, dcsidxnc2_gpu, dcsidync2_gpu, dcsidxc2_gpu, dcsidyc2_gpu, calorically_perfect, rgas0,t0)
    integer, intent(in), value :: start_or_end, nr_type, nx, ny, nz, ng, nv, indx_cp_l, indx_cp_r, calorically_perfect
    real(rkind), intent(in), value :: rgas0,t0
    real(rkind), dimension(3) :: c_one
    real(rkind), dimension(5) :: dw_dn, dwc_dn, ev, dw_dn_outer, dwc_dn_outer
    ...
    dcsixjdcsi = sgn_dw * (c_one(1)*dcsidxc2_gpu(i,j)/jac_gpu(i,j)+c_one(2)*dcsidxc2_gpu(i+1*sgn_dw,&
    &j)/jac_gpu(i+1*sgn_dw,j)+ c_one(3)*dcsidxc2_gpu(i+2*sgn_dw,j)/jac_gpu(i+2*sgn_dw,j))
    dcsiyjdcsi = sgn_dw * (c_one(1)*dcsidyc2_gpu(i,j)/jac_gpu(i,j)+c_one(2)*dcsidyc2_gpu(i+1*sgn_dw,&
    &j)/jac_gpu(i+1*sgn_dw,j)+ c_one(3)*dcsidyc2_gpu(i+2*sgn_dw,j)/jac_gpu(i+2*sgn_dw,j))
    pp = p_rho * rho
  ...
  attributes(global) subroutine bc_nr_lat_y_c2_kernel(start_or_end, nr_type, nx, ny, nz, ng, nv,&
  & w_aux_gpu, w_gpu, fl_gpu, detady_gpu, indx_cp_l, indx_cp_r, cp_coeff_gpu, wfar_gpu, jac_gpu,&
  & meta_gpu, detadxnc2_gpu, detadync2_gpu, detadxc2_gpu, detadyc2_gpu, wall_tag_gpu,&
  & calorically_perfect,rgas0,t0)
    integer, intent(in), value :: start_or_end, nr_type, nx, ny, nz, ng, nv, indx_cp_l, indx_cp_r, calorically_perfect
    real(rkind), intent(in), value :: rgas0,t0
    real(rkind), dimension(3) :: c_one
  ...
  & rand_type)
    ...
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu, cp_coeff_gpu
  ...
  subroutine bcfree_sub_cuf(ilat, nx, ny, nz, ng, w_gpu, w_aux_gpu, aoa, t0, ptot0, ttot0, rgas0,&
  & indx_cp_l, indx_cp_r, cv_coeff_gpu, cp_coeff_gpu, calorically_perfect, tol_iter_nr)
    integer, intent(in) :: ilat,nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
  ...
  subroutine bcextr_var_cuf(nx, ny, nz, ng, w_gpu)
    integer :: nx,ny,nz,ng
    integer :: ilat
    integer :: i,j,k,l,m,iercuda
    real(rkind), dimension(1-ng:nx+ng, 1-ng:ny+ng, 1-ng:nz+ng, 1:1), intent(inout), device :: w_gpu
  ...
  subroutine bcextr_airfoil_var_cuf(nx, ny, nz, ng, ndim, wall_tag_gpu, w_gpu, ileftx, irightx, ileftz, irightz)
  ...
  endsubroutine bcextr_airfoil_var_cuf
  ...
  subroutine bcsym_c2_cuf(ilat, nx, ny, nz, ng, w_gpu, dxdcsic2_gpu, dydcsic2_gpu)
    integer :: nx,ny,nz,ng
    integer :: ilat
    integer :: i,k,l, iercuda
    real(rkind), dimension(1-ng:, 1-ng:, 1-ng:, 1:), intent(inout), device :: w_gpu
    real(rkind), dimension(1-ng:, 1-ng:), intent(in), device :: dxdcsic2_gpu
    real(rkind), dimension(1-ng:, 1-ng:), intent(in), device :: dydcsic2_gpu
  ...
  subroutine bcwall_airfoil_cuf(ilat, nx, ny, nz, ng, twall, w_gpu, w_aux_gpu, wall_tag_gpu,&
  & indx_cp_l, indx_cp_r, cv_coeff_gpu, t0, rgas0, calorically_perfect, tol_iter_nr, xc2_gpu, yc2_gpu,&
  & a_tw, v_bs, thic, kx_tw, om_tw, xtw1, xtw2, time, dxdetanc2_gpu, dydetanc2_gpu, u0)
    integer :: nx,ny,nz,ng,indx_cp_l,indx_cp_r,calorically_perfect
    integer :: ilat
    integer :: i,k,l, iercuda
    integer, dimension(1-ng:nx+ng), intent(in), device :: wall_tag_gpu
    ...
    real(rkind), dimension(1-ng:, 1-ng:, 1-ng:, 1:), intent(in), device :: w_aux_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cv_coeff_gpu
    real(rkind), intent(in), dimension(1-ng:nx+ng,1-ng:ny+ng), device :: xc2_gpu, yc2_gpu
    real(rkind), intent(in), dimension(1-ng:, 1-ng:), device :: dxdetanc2_gpu, dydetanc2_gpu
  ...
  endsubroutine bcwall_airfoil_cuf
  ...
  subroutine compute_airfoil_forces_runtime_cuf(nx, ny, nz, ng, nv, p0, u0, rgas0, w_aux_gpu,&
  & meta_gpu, csimod_gpu, wall_tag_gpu, dxdcsic2_gpu, dydcsic2_gpu, n, a, pn, pa, tn, ta)
    ...
    real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), device :: w_aux_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: meta_gpu, csimod_gpu
    integer, dimension(1-ng:nx+ng), intent(in), device :: wall_tag_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dxdcsic2_gpu, dydcsic2_gpu
    ...
    tn = 0._rkind
    ta = 0._rkind
    !$cuf kernel do(3) <<<*,*>>> reduce(+:n, a, pn, pa, tn, ta)
    do k=1,nz
      do j=1,1
        do i=1,nx
          if(wall_tag_gpu(i) < 1) then
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine compute_airfoil_forces_runtime_cuf
  ...
  subroutine compute_dt_c2_cuf(nx, ny, nz, ng, rgas0, prandtl, dcsidxnc2_gpu, dcsidync2_gpu,&
  ...
  & indx_cp_l, indx_cp_r, cp_coeff_gpu,fluid_mask_gpu,calorically_perfect,t0)
    ...
    integer, dimension(1-ng:,1-ng:,1-ng:), intent(in), device :: fluid_mask_gpu
    real(rkind), dimension(indx_cp_l:indx_cp_r+1), intent(in), device :: cp_coeff_gpu
    real(rkind), dimension(1:), intent(in), device :: dzitdz_gpu, dzitdzs_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: dcsidxnc2_gpu, dcsidync2_gpu, detadxnc2_gpu, detadync2_gpu
    real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: mcsi_gpu, meta_gpu
    real(rkind) :: dtxi, dtyi, dtzi, dtxv, dtyv, dtzv, dtxk, dtyk, dtzk
    integer :: i,j,k,ll,iercuda
    real(rkind) :: rho, ri, uu, vv, ww, tt, mu, nu, k_over_rhocp, c
    real(rkind) :: dtxi_max, dtyi_max, dtzi_max, dtxv_max, dtyv_max, dtzv_max, dtxk_max, dtyk_max, dtzk_max
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine compute_dt_c2_cuf
  ...
  subroutine tripping_pressure_cuf(nx,ny,nz,ng,nv,i_rank_start,pi,itr1,itr2,x0tr,y0tr,x0ts,y0ts,&
  ...
  & dxdetanc2_gpu, dydetanc2_gpu)
    ...
    integer, dimension(1-ng:nx+ng), intent(in), device :: wall_tag_gpu
    real(rkind), intent(in), dimension(1-ng:, 1-ng:), device :: dxdetanc2_gpu, dydetanc2_gpu
    real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng), intent(in), device :: xc2_gpu, yc2_gpu
    real(rkind), dimension(1-ng:), intent(in), device :: z_gpu
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine tripping_pressure_cuf
  subroutine tripping_suction_cuf(nx,ny,nz,ng,nv,i_rank_start,pi,its1,its2,x0tr,y0tr,x0ts,y0ts,lamx,&
  ...
  & dydetanc2_gpu)
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine tripping_suction_cuf
  ...
  attributes(global) launch_bounds(256) subroutine insitu_swirling_kernel(nv, nx, ny, nz,&
  & visc_order, ng, npsi,mpsi,dcsidx_gpu, detady_gpu, dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, psi_gpu,&
  & u0, x_gpu )
    implicit none
  ...
  attributes(global) launch_bounds(256) subroutine insitu_swirling_c2_kernel(nv, nx, ny, nz,&
  & visc_order, ng, npsi,mpsi,dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, dcsidxc2_gpu, detadyc2_gpu,&
  & detadxc2_gpu, dcsidyc2_gpu, psi_gpu, u0, x_gpu, vis_tag_gpu, wall_tag_gpu)
    implicit none
    integer, value :: nv, nx, ny, nz, visc_order, ng, npsi,mpsi
    real(rkind), value :: u0
    real(rkind), dimension(nz) :: dzitdz_gpu
  ...
  attributes(global) launch_bounds(256) subroutine insitu_schlieren_kernel(nv, nx, ny, nz,&
  & visc_order, ng, npsi,mpsi,dcsidx_gpu, detady_gpu, dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, psi_gpu,&
  & u0, x_gpu )
    implicit none
    integer, value :: nv, nx, ny, nz, visc_order, ng, npsi,mpsi
  ...
  attributes(global) launch_bounds(256) subroutine insitu_schlieren_c2_kernel(nv, nx, ny, nz,&
  & visc_order, ng, npsi,mpsi,dzitdz_gpu, w_aux_gpu, coeff_deriv1_gpu, dcsidxc2_gpu, detadyc2_gpu,&
  & detadxc2_gpu, dcsidyc2_gpu, psi_gpu, u0, x_gpu, vis_tag_gpu, wall_tag_gpu)
    implicit none
    integer, value :: nv, nx, ny, nz, visc_order, ng, npsi,mpsi
  ...
  subroutine insitu_div_cuf(nx, ny, nz, ng, visc_order, npsi,mpsi, w_aux_gpu, coeff_deriv1_gpu,&
  & dcsidx_gpu, detady_gpu, dzitdz_gpu, psi_gpu )
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine insitu_div_cuf

  subroutine insitu_omega_cuf(nx, ny, nz, ng, visc_order, npsi,mpsi, w_aux_gpu, coeff_deriv1_gpu,&
  & dcsidx_gpu, detady_gpu, dzitdz_gpu, psi_gpu )
  ...
  subroutine insitu_ducros_cuf(nx, ny, nz, ng, visc_order, npsi,mpsi, u0,l0, w_aux_gpu,&
  ...
  subroutine compute_tspec_cuf(nx, ny, nz, ng, ndft, j_slice, i_win, it_win, w_aux_gpu, w_tspec_gpu)
    integer, intent(in) :: nx, ny, nz, ng, ndft, j_slice, i_win, it_win
    real(rkind), dimension(1-ng:,1-ng:,1-ng:,1:), intent(in), device :: w_aux_gpu
    real(rkind), dimension(1:,1:,1:,1:,1:), intent(inout), device :: w_tspec_gpu
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine compute_tspec_cuf

  subroutine compute_psd_tspec_cuf(nx, ny, nz, ndft, i_win, dt_tspec, w_tspec_gpu, w_psd_tspec_gpu)
    integer, intent(in) :: nx, ny, nz, ndft, i_win
    real(rkind), intent(in) :: dt_tspec
    real(rkind), dimension(1:,1:,1:,1:,1:), intent(inout), device :: w_tspec_gpu
    real(rkind), dimension(1:,1:), intent(inout), device :: w_psd_tspec_gpu
    integer :: i,j,k,iercuda, ll, l, nn, n
    real(rkind) :: pi
    pi = acos(-1._rkind)
    !$cuf kernel do(2) <<<*,*>>>
    ...
    enddo
    !@cuf iercuda=cudadevicesynchronize()
  endsubroutine compute_psd_tspec_cuf

  subroutine compute_wallprop_c2_cuf(nx,ny,nz,ng,w_aux_gpu,wallprop_gpu,dxdcsic2_gpu,dydcsic2_gpu,csimod_gpu,meta_gpu,wall_tag_gpu)
    integer, intent(in) :: nx,ny,nz,ng
    integer, dimension(1-ng:nx+ng), intent(in), device :: wall_tag_gpu
    real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(in), device :: w_aux_gpu
    real(rkind), dimension(1-ng:,1-ng:, 2:), intent(inout), device :: wallprop_gpu
  ...
  endsubroutine compute_wallprop_c2_cuf

!==========================================================================================
! LES (WALE subgrid-scale model) — ported from streams_wmles_forcing, no wall model / no IBM
!==========================================================================================
    subroutine eval_velaux_cuf(nx,ny,nz,ng,w_gpu,w_aux_gpu)
        integer(ikind), intent(in) :: nx, ny, nz, ng
        real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(in),  device   :: w_gpu
        real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_aux_gpu
        integer(ikind) :: i, j, k, ll
        integer(ikind) :: iercuda
        real(rkind) :: rho,rhou,rhov,rhow,ri,uu,vv,ww
!
        !$cuf kernel do(3) <<<*,*>>>
        do k=1-ng,nz+ng
         do j=1-ng,ny+ng
          do i=1-ng,nx+ng
              ...
              w_aux_gpu(i,j,k,3) = vv
              w_aux_gpu(i,j,k,4) = ww
              w_aux_gpu(i,j,k,11) = 0._rkind
              w_aux_gpu(i,j,k,12) = 0._rkind
          enddo
         enddo
        enddo
        !@cuf iercuda=cudaDeviceSynchronize()
!
    endsubroutine eval_velaux_cuf
!
!   LES
!
    subroutine les_wale_mut_cuf(nx,ny,nz,ng,ep_order,w_aux_gpu,coeff_deriv1_gpu,dcsidx_gpu, &
                                detady_gpu,dzitdz_gpu,les_c_wale,les_c_yoshi,u0,l0)

        integer, intent(in) :: nx,ny,nz,ng,ep_order
        real(rkind), intent(in) :: les_c_wale,les_c_yoshi,u0,l0
        real(rkind), dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_aux_gpu
        real(rkind), dimension(1:,1:), intent(in), device :: coeff_deriv1_gpu
        real(rkind), dimension(1:), intent(in), device :: dcsidx_gpu, detady_gpu, dzitdz_gpu
        real(rkind) :: ccl, delta2
        real(rkind) :: mu_sgs,tt,cploc,rho,nu_sgs,rhokmu
        ...
        do k=1,nz
         do j=1,ny
          do i=1,nx
           ...
           do l = 1,lmax
            ccl = coeff_deriv1_gpu(l,lmax)
            dudx = dudx + ccl*(w_aux_gpu(i+l,j,k,2) - w_aux_gpu(i-l,j,k,2))
            dudy = dudy + ccl*(w_aux_gpu(i,j+l,k,2) - w_aux_gpu(i,j-l,k,2))
            dudz = dudz + ccl*(w_aux_gpu(i,j,k+l,2) - w_aux_gpu(i,j,k-l,2))
            dvdx = dvdx + ccl*(w_aux_gpu(i+l,j,k,3) - w_aux_gpu(i-l,j,k,3))
            dvdy = dvdy + ccl*(w_aux_gpu(i,j+l,k,3) - w_aux_gpu(i,j-l,k,3))
            dvdz = dvdz + ccl*(w_aux_gpu(i,j,k+l,3) - w_aux_gpu(i,j,k-l,3))
            dwdx = dwdx + ccl*(w_aux_gpu(i+l,j,k,4) - w_aux_gpu(i-l,j,k,4))
            dwdy = dwdy + ccl*(w_aux_gpu(i,j+l,k,4) - w_aux_gpu(i,j-l,k,4))
            dwdz = dwdz + ccl*(w_aux_gpu(i,j,k+l,4) - w_aux_gpu(i,j,k-l,4))
           enddo
           ...
           omegaz = dvdx-dudy
           omod2 = omegax*omegax+omegay*omegay+omegaz*omegaz
           w_aux_gpu(i,j,k, 9) = (max(-div/sqrt(omod2+div**2+(u0/l0)**2),0._rkind))**2
           ...
           nu_sgs = mu_sgs/rho
           w_aux_gpu(i,j,k,11)  = mu_sgs
           !rhokmu = rho*k_sgs/mu_sgs/3._rkind
           rhokmu = les_c_yoshi*nu_sgs/delta2/3._rkind
           w_aux_gpu(i,j,k,12) = rhokmu
           w_aux_gpu(i,j,k,10) = div3l+rhokmu
          enddo
         enddo
        enddo
        !@cuf iercuda=cudadevicesynchronize()
!
    endsubroutine les_wale_mut_cuf

    subroutine les_wale_mut_c2_cuf(nx,ny,nz,ng,visc_order,w_aux_gpu,coeff_deriv1_gpu,dzitdz_gpu, &
                                   dcsidxc2_gpu, detadxc2_gpu, dcsidyc2_gpu, detadyc2_gpu, &
                                   wall_tag_gpu,vis_tag_gpu,jac_gpu,les_c_wale,les_c_yoshi,u0,l0)
        ...
        real(rkind), dimension(1-ng:,1-ng:), intent(in), device :: detadxc2_gpu, dcsidyc2_gpu
        integer, dimension(1-ng:), intent(in), device :: vis_tag_gpu, wall_tag_gpu
        real(rkind), dimension(1-ng:nx+ng,1-ng:ny+ng), device :: jac_gpu
        real(rkind) :: cli, clj, clk, delta2
        real(rkind) :: ucsi, vcsi, wcsi, ueta, veta, weta, uzit, vzit, wzit
        real(rkind) :: mu_sgs,tt,cploc,rho,nu_sgs,rhokmu
        real(rkind) :: normS, normSd
        real(rkind) :: dudx, dudy, dudz
        real(rkind) :: dvdx, dvdy, dvdz
...
!
           ! Compute local grid scale
           delta2 = (1._rkind/jac_gpu(i,j)*dzitdz_gpu(k))
           delta2 = delta2**(-2._rkind/3._rkind)
!
           lmaxi = lmax
           lmaxj = lmax
           if (j == 1) lmaxi = vis_tag_gpu(i) ! reducing order in the nodes near TE
           if (wall_tag_gpu(i) < 1) lmaxj = min(j,lmax) ! reducing accuracy at the wall (only airfoil)
!
           ucsi    = 0._rkind
           vcsi    = 0._rkind
           wcsi    = 0._rkind
           ...
           dudx  = ucsi *dcsidxc2_gpu(i,j) + ueta *detadxc2_gpu(i,j)
           dvdx  = vcsi *dcsidxc2_gpu(i,j) + veta *detadxc2_gpu(i,j)
           dwdx  = wcsi *dcsidxc2_gpu(i,j) + weta *detadxc2_gpu(i,j)
           dudy  = ucsi *dcsidyc2_gpu(i,j) + ueta *detadyc2_gpu(i,j)
           dvdy  = vcsi *dcsidyc2_gpu(i,j) + veta *detadyc2_gpu(i,j)
           dwdy  = wcsi *dcsidyc2_gpu(i,j) + weta *detadyc2_gpu(i,j)
           dudz  = uzit *dzitdz_gpu(k)
           dvdz  = vzit *dzitdz_gpu(k)
           dwdz  = wzit *dzitdz_gpu(k)
        ...
        !@cuf iercuda=cudadevicesynchronize()
!
    endsubroutine les_wale_mut_c2_cuf
!
    subroutine eval_aux_les_cuf(nx, ny, nz, ng, w_gpu, w_aux_gpu, &
            visc_model, mu0, t0, sutherland_S, T_ref_dim, &
            powerlaw_vtexp, VISC_POWER, VISC_SUTHERLAND, VISC_NO, Prandtl, cp_coeff_gpu, &
            cv_coeff_gpu, indx_cp_l, indx_cp_r, rgas0, calorically_perfect, tol_iter_nr, les_pr)
        integer(ikind), intent(in) :: nx, ny, nz, ng, visc_model
        integer(ikind), intent(in) :: VISC_POWER, VISC_SUTHERLAND, VISC_NO, indx_cp_l, indx_cp_r, calorically_perfect
        real(rkind),    intent(in) :: mu0, t0, sutherland_S, T_ref_dim, powerlaw_vtexp, rgas0, tol_iter_nr, Prandtl,les_pr
        real(rkind),    dimension(indx_cp_l:indx_cp_r+1), intent(in),  device   :: cv_coeff_gpu,cp_coeff_gpu
        real(rkind),    dimension(1-ng:,1-ng:,1-ng:, 1:), intent(in),  device   :: w_gpu
        real(rkind),    dimension(1-ng:,1-ng:,1-ng:, 1:), intent(inout), device :: w_aux_gpu
        integer(ikind)                        :: i, j, k, ll
        real(rkind)                           :: rho, rhou, rhov, rhow, rhoe, ri, uu, vv, ww, qq, pp, tt
        real(rkind)                           :: mu, ee, cploc, mu_sgs, nu_sgs, k_sgs, rhokmu, delta2
        integer(ikind)                        :: iercuda
        ...
        do k=1-ng,nz+ng
         do j=1-ng,ny+ng
          do i=1-ng,nx+ng
              ...
              vv     = w_aux_gpu(i,j,k,3)
              ww     = w_aux_gpu(i,j,k,4)
              mu_sgs = w_aux_gpu(i,j,k,11)
              nu_sgs = mu_sgs/rho
              k_sgs  = 3._rkind*w_aux_gpu(i,j,k,12)*nu_sgs
              rhoe   = w_gpu(i,j,k,5)
              qq     = 0.5_rkind*(uu*uu+vv*vv+ww*ww)
              ee     = rhoe/rho-qq-k_sgs
              ...
              mu_sgs = min(mu_sgs,200._rkind*mu)
              w_aux_gpu(i,j,k,7) = mu+mu_sgs
              if (calorically_perfect==1) then
               cploc = cp_coeff_gpu(0)
              else
               cploc = 0._rkind
              ...
              w_aux_gpu(i,j,k,8) = mu*cploc/Prandtl+mu_sgs*cploc/les_pr
